...
class Immunization extends CI_Controller {
	...
	public function index()		{
		...
		if($userdata)	{
			...
			$data['title'] 		= 'Pending Immunization Requests';
			...
			//Paginated data		            
	   		$config['num_links'] = 5;
			$config['base_url'] = base_url('immunization/index');
			$config["total_rows"] = $this->immunization_model->count_immu($search, 0);
			$config['per_page'] = 20;		
		    ...
		    $data["results"] = $this->immunization_model->fetch_immu($config["per_page"], $page, $search, 0);
			...
			$this->load->view('immunization/list', $data);				
		   ...
		   if($this->form_validation->run() == FALSE)	{
				...
				if($this->immunization_model->update_status($status, $immu)) {
					...
					if($status == 1) {
						...
						$this->billing_model->add_item($billing['id'], $case['service'], 1, $case['amount'], 'IMMU REQ #'.prettyID($immu)); //add Service in the current Billing
